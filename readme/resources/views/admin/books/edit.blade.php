...
<div class="container mx-auto py-4">
    ...
    <form action="{{ route('admin.books.update', $book) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" class="border rounded w-full px-3 py-2" value="{{ old('title', $book->title) }}" required>
        </div>
        <div class="mb-4">
            <label for="summary" class="block font-medium">Résumé</label>
            <textarea name="summary" id="summary" class="border rounded w-full px-3 py-2">{{ old('summary', $book->summary) }}</textarea>
        </div>
        <div class="mb-4">
            <label for="year" class="block font-medium">Année</label>
            <input type="number" name="year" id="year" class="border rounded w-full px-3 py-2" value="{{ old('year', $book->year) }}">
        </div>
        <div class="mb-4">
            <label for="stock" class="block font-medium">Stock</label>
            <input type="number" name="stock" id="stock" class="border rounded w-full px-3 py-2" value="{{ old('stock', $book->stock) }}" required>
        </div>
        <div class="mb-4">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($authors as $author)
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="authors[]" value="{{ $author->id }}" @checked(collect(old('authors', $book->authors->pluck('id')))->contains($author->id))>
